...
export async function main(ns: NS): Promise<void> {
    ...
    if (!ns.getPlayer().hasCorporation) {
        ns.corporation.createCorporation("Tenable", false);
    }
    ...
    while (true) {
        ...
        for (const division of corp.divisions.reverse()) {
            // check for industry types which don't allow products or certain upgrades, exclude them from script
            if (!noProduct.includes(ns.corporation.getDivision(division.name).type)) {
...
async function hireEmployees(ns: NS, division: Division, productCity = "Sector-12") {
    let employees = ns.corporation.getOffice(division.name, productCity).employees.length;
...
async function trickInvest(ns: NS, division: Division, productCity = "Sector-12") {
...
function newProduct(ns: NS, division: Division) {
    ...
    for (const product of division.products) {
        if (ns.corporation.getProduct(division.name, product).developmentProgress < 100) {
            allProductNumbers.push(product.charAt(product.length - 1));
            underDevelopment.push(product.charAt(product.length - 1));
            ...
            allProductNumbers.push(product.charAt(product.length - 1));
            developedProducts.push(product.charAt(product.length - 1));
    ...
    if (allProductNumbers.length > 0) {
        newProductNumber = parseInt(allProductNumbers[allProductNumbers.length - 1]) + 1;
        // cap product numbers to one digit and restart at 0 if > 9.
        if (newProductNumber > 9) {
            newProductNumber = 0;
        }
    }
    const newProductName = "Product-v" + newProductNumber;
    ...
    if (allProductNumbers.length < numProducts) {
...
async function initCities(ns: NS, division: Division, productCity = "Sector-12") {
    for (const city of cities) {
    ...
    ns.corporation.makeProduct(division.name, productCity, "Product-v0", 1e9, 1e9);
}
...
const noProduct = ["Agriculture", "Chemical", "Mining", "Fishing", "Utilities", "Energy"];