...
export async function main(ns) {
    ...
    var scriptram = ns.getScriptRam('/backup/worm.js', 'home'); //get ram for this script
    var hackscriptram = ns.getScriptRam('/backup/hackservers.js', 'home'); //get ram for hack script
    ...
    if (hsthreads) { //if usethreads exists for this script, build args array of parameters based on this scripts args
        ...
        if (ns.getHostname() != 'home') { //copy hack script to this server and spawn script with threads and arguments as a single string
            await ns.scp('/backup/hackservers.js', 'home', ns.getHostname());
        }
        ns.spawn('/backup/hackservers.js', hsthreads, hsargs.toString());
    }
    ...
    async function worm(server) {
        ...
        if (!ns.fileExists('/backup/worm.js', server)) {
            ns.print('/backup/worm.js being copied to ' + server);
            await ns.scp('/backup/worm.js', 'home', server);
        }

        //if you don't see either script running on target server, run worm on it.
        if (!ns.scriptRunning('/backup/worm.js', server) && !ns.scriptRunning('/backup/hackservers.js', server)) {
            ns.print('running worm on ' + server);
            await ns.sleep(11000);
            await ns.scp('/backup/worm.js', 'home', server);
            ns.exec('/backup/worm.js', server, 1, ...ogArgs);
        }
    }
}